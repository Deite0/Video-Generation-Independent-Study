...
class PreProcessVideos:
    ...
    def video_processor(
        ...
        frame_number = (
            random.randrange(2, int(num_frames)) if random_start_frame else frame_num
            )
        frame = video_reader[frame_number].permute(-1,-2,-3)
    ...
    def video_processor(
        ...
        frame = video_reader[frame_number].permute(2, 1, 0)  # Assuming input shape is (H, W, C)
        # Convert tensor to PIL image
        pil_image = transforms.ToPILImage()(frame.byte().clamp(0, 255))
        # Convert to RGB if necessary
        if pil_image.mode != 'RGB':
            pil_image = pil_image.convert('RGB')
        return frame_number, pil_image
    ...
    def process_blip(self, image: Image):
        ...
        print(f" process_blip inputs {inputs}.") 
        ...
        print(f" process_blip generated_ids {generated_ids}.") 
        generated_text = self.processor.batch_decode(
            generated_ids, 
            skip_special_tokens=True)[0].strip()
        
        print(f" process_blip generated_text {generated_text}.") 
            ...
            for video in files:
                ...
                if video.endswith(self.vid_types):
                    ...
                    try:
                        ...
                        print(f"trying video length {video_len}.")    
                    except:
                    ...
                    try:
                        ...
                        print(f"starting second time trying video {video_config}.")
